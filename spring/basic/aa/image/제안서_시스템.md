...
| qualityAttr-02| 통합 데이터 관리| 보안성, 변경가능성| 상| 상| systemFeature-01|
...
| qualityAttr-05| 유지보수에 적은 비용이 소모| 변경가능성, 유지보수성| 상| 중| systemFeature-02, systemFeature-03|
...
| --| --| --|--|
| architectureDriver-01| 대용량 데이터 처리 방식| 사용성, 변경가능성| qualityAttr-01, qualityAttr-02| 
| architectureDriver-02| 추가 기능에 대한 영향도 최소화| 편의성, 변경가능성| qualityAttr-03, qualityAttr-05| 
| architectureDriver-03| 테스트 방식 (유지보수 비용 최소화)| 유지보수성| qualityAttr-05| 
...
* 결정사항: DBMS로 일반적인 데이터 처리를 수행하고, 검색과 분석에 필요한 필드만 커스텀 매핑으로 ElasticSearch 에 동기화 하는 방식으로 데이터베이스를 보조하는 search engine으로 사용
