...
### 애자일 방법론 VS 기존 방법론
> 기존 방법론은 지나치게 계획에 의존하고 있으며 형식적인 절차를 따르기 때문에 시간도 오래걸리고 그 효율성이 크게 저하된다는 단점이 있다.
...
```text
[개인적인 생각]
이번에 강의에서 가장 집중하지 못했던 이유기도 한데 애자일 방법론이 무조건 옳다고 생각하지 않고, 문서가 필요없다고 생각하지도 않지만
아무런 코딩없이 문서만 100장 넘게 작성하는 방식은 너무 불필요하며, 옛날 방식의 작업으로 느껴졌습니다.
```

## 이번 교육을 받으면서 개발중인 코드 
[제안서 검색 시스템](https://github.com/kim-taehan/proposal)
: https://github.com/kim-taehan/proposal (AA 교육에서 발표했던 시스템 개발중이 코드)
